    ...
    if (phone) {
        ...
        handlers._tokens.verifyToken(token, phone, function(tokenIsValid) {
            if (tokenIsValid) {
                ...
                callback(403, { Error: `Missing requirde token in header, or token is invalid` });
            }
        });
    } else {
                                        ...
                                        _data.delete(`checks`, checkId, function(err) {
                                            ...
                                            if (checksDeleted === checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    callback(500, { Error: `Errors encountered while deleting all of the user's checks. All checks may have not been deleted successfully` });
                                                }
                                            }
                                        });
                                    });
...
handlers._checks.put = function(data, callback) {
    ...
    if (id) {
        // Check for one optional fild or more
        if (protocol || url || method || successCodes || timeoutSeconds) {
            // Lookup the check
            _data.read(`checks`, id, function(err, checkData) {
                if (!err && checkData) {
                                ...
                                _data.read(`users`, checkData.userPhone, function(err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, { Error: `Could not find the check on the user's checks so could not remove it.` });
                                        }
                                    } else {
